...
public class CameraManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        // ... (translation checks)
        if (Math.Abs(Input.mouseScrollDelta.y) > 0f) _Zoom(Input.mouseScrollDelta.y > 0f ? 1 : -1);
    ...
    private void _TranslateCamera(int dir)
    {
        ...
        if (Physics.Raycast(_ray, out _hit, 1000f, Globals.TERRAIN_LAYER_MASK)) transform.position = _hit.point + Vector3.up * altitude;
        _FixAltitude();
